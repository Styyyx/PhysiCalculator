...
$('.panel#speed .buttons #btnCalculate').on('click', function () {
    if (IsFormComplete('.panel#speed .form')) {
      ...
      if (isSpeed)
        switch (methodIndex) {
          case 0: {
            ShowResult(GetSpeed(data.dist, data.speedTime), 'meter(s)/second');
            break;
          }
          case 1: {
            ShowResult(GetDist(data.speed, data.speedTime), 'meter(s)');
            break;
          }
          case 2: {
        ...
        switch (methodIndex) {
          case 0: {
            ShowResult(GetFinalVelocity(data.vi, data.acc, data.velocityTime), 'meters');
            break;
          }
          case 1: {
            ShowResult(GetInitialVelocity(data.vf, data.acc, data.velocityTime), 'seconds');
            break;
          }
          case 2: {
            ShowResult(GetAcceleration(data.vf, data.vi, data.velocityTime), 'meters');
            break;
          }
          case 3: {
            ShowResult(GetTime(data.vf, data.vi, data.acc), 'meters');
          }
        }
      }
